    ...
    public class MaterialController : Controller
    {
        ...
        public async Task<JsonResult> GetList()
        {
            try
            {
                ...
                var list = await _materialService.GetAllAsync(pageNumber, pageSize, m => m.MaterialId,
        ...
        public async Task<JsonResult> Get(int id)
        {
            try
            {
                var material = await _materialService.FindAsync(m=>m.MaterialId == id);
                if (material != null)
                {
        ...
        public async Task<JsonResult> Update(Material material, int id)
        {
            ...
            try
            {
                var checkMaterial = await _materialService.FindAsync(m => m.MaterialId == id);
                if (checkMaterial != null & (material.Status.Equals(Status.Inactive) || material.Status.Equals(Status.Active)))
                {
                    checkMaterial.MaterialName = material.MaterialName;
                    checkMaterial.MaterialPrice = material.MaterialPrice;
